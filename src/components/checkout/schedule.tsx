...
export default function Schedule() {
  ...
  const [cutlery, setCutlery] = useState('yes');
  ...
  return (
    <div className="w-full">
      <div className="w-full mx-auto">
        ...
        <div className="text-gray-700 my-7">
          DO YOU NEED PLASTIC FORK AND SPOON?
        </div>
        <RadioGroup value={cutlery} onChange={setCutlery}>
          <RadioGroup.Label className="sr-only">
            {t('text-cutlery')}
          </RadioGroup.Label>
          <div className="flex gap-x-3">
            <RadioGroup.Option
              ...
              className={({ active, checked }) =>
                cn(
                  'relative rounded-xl px-3 py-1 cursor-pointer focus:outline-none',
                  checked ? 'bg-brand text-brand-light' : 'bg-gray-100'
                )
              }
            >
