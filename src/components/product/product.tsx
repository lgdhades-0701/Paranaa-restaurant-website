...
const ProductSingleDetails: React.FC = () => {
  ...
  const { price, basePrice, discount } = usePrice(
    data && {
      amount: data?.[0]?.sale_price ? data[0].sale_price : data[0].price,
      baseAmount: data[0]?.price,
      currencyCode: 'THB',
    }
  );
  const handleChange = () => {
      ...
      <div className="grid-cols-10 lg:grid gap-7 2xl:gap-8">
        <div className="col-span-5 mb-6 overflow-hidden xl:col-span-6 md:mb-8 lg:mb-0">
          {!!data?.[0]?.gallery?.length ? (
            ...
            <div className="flex items-center justify-center w-auto">
              <Image
                src={data?.[0]?.image?.[0].original ?? '/product-placeholder.svg'}
                alt={data?.name!}
                width={900}
                height={680}
              />
        ...
        <div className="flex flex-col col-span-5 shrink-0 xl:col-span-4 xl:ltr:pl-2 xl:rtl:pr-2">
          <div className="pb-3 lg:pb-5">
            ...
            {data?.[0]?.unit && isEmpty(variations) ? (
