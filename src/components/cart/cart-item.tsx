...
const CartItem: React.FC<CartItemProps> = ({ item }) => {
  ...
  return (
    ...
    <div
      ...
      title={item?.name}
    >
      <div className="relative flex rounded overflow-hidden shrink-0 cursor-pointer w-[70px] md:w-[100px] h-[70px] md:h-[100px]">
        <Image
          src={item?.image?.[0]?.thumbnail ?? '/assets/placeholder/cart-item.svg'}
          width={100}
          height={100}
          loading="eager"
      ...
      <div className="flex items-start justify-between w-full overflow-hidden">
        <div className="ltr:pl-3 rtl:pr-3 md:ltr:pl-4 md:rtl:pr-4">
          <Link
            ...
            className="block bold leading-5 transition-all text-brand-dark text-13px sm:text-sm lg:text-15px hover:text-brand"
          >
          </Link>
          <Link
            href={`${ROUTES.PRODUCT}/${item?.slug}`}
            className="block leading-5 transition-all text-brand-dark text-bold text-13px sm:text-sm lg:text-15px hover:text-brand"
          >
            {item?.name}
          </Link>
